...
Notice that this test uses a real Redux store instead of [redux-mock-store](https://github.com/dmitry-zaets/redux-mock-store). **Using a real Redux store in your test effectively closes the loop between UI event and UI update.** In most cases, your tests can use the same store configuration that is used in your application.
